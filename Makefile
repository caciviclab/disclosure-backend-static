...
do-import-spreadsheets:
	...
	csvsql --db postgresql:///$(DATABASE_NAME) --insert $(CSV_PATH)/candidates.csv --no-create
	...
	csvsql --db postgresql:///$(DATABASE_NAME) --insert $(CSV_PATH)/referendums.csv --no-create
	...
	csvsql --db postgresql:///$(DATABASE_NAME) --insert $(CSV_PATH)/name_to_number.csv --no-create

	echo 'DROP TABLE IF EXISTS committees;' | psql $(DATABASE_NAME)
	./bin/create-table $(DATABASE_NAME) $(CSV_PATH) committees
	csvsql --db postgresql:///$(DATABASE_NAME) --insert $(CSV_PATH)/committees.csv --no-create
	...
	csvsql --db postgresql:///$(DATABASE_NAME) --insert downloads/csv/office_elections.csv --no-create
	...
	csvsql --db postgresql:///$(DATABASE_NAME) --insert downloads/csv/elections.csv --no-create
